...
export function DirectoryCtaSection() {
  return (
    <section className="relative w-full overflow-hidden border-t border-[#111] bg-[#050505] py-24 md:py-32 flex flex-col items-center justify-center">
      <style>{`
      ...
      <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.04)_0%,transparent_60%)] pointer-events-none" />

      {/* Marquee Background */}
      <div className="absolute inset-0 z-0 flex flex-col justify-center gap-6 opacity-[0.25] pointer-events-none select-none [mask-image:linear-gradient(to_bottom,transparent,black_20%,black_80%,transparent)]">
        {/* Row 1 */}
      ...
      <div className="absolute inset-0 z-0 pointer-events-none shadow-[inset_0_0_150px_100px_#050505]" />

      {/* Content */}
      <div className="relative z-10 flex flex-col items-center text-center px-4 max-w-4xl mx-auto mt-4">
        <h2 className="text-[clamp(52px,10vw,140px)] font-bold tracking-[-0.06em] leading-[0.9] text-[rgba(255,255,255,0.82)] mb-12">
          Looking for <br className="hidden md:block" />
          <span className="text-white/40 italic font-medium tracking-tight">someone?</span>
        </h2>
        
        <Link 
          href="/members" 
          className="group relative inline-flex h-16 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-white/[0.02] px-10 backdrop-blur-md transition-all duration-300 hover:bg-white/[0.08] hover:border-white/20 active:scale-95"
        >
          <span className="relative flex items-center gap-3 text-white/90 group-hover:text-white transition-colors" style={{ fontFamily: "'Satoshi','Inter',sans-serif", fontSize: '1.25rem', fontWeight: 500, letterSpacing: '-0.01em' }}>
            Member Directory
